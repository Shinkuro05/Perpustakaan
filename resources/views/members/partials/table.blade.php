    ...
    <table class="w-full">
        ...
        <tbody class="divide-y divide-slate-50">
            ...
            <tr class="hover:bg-slate-50 transition-colors">
                <td class="px-6 py-4">
                    <span class="font-mono text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded-lg">{{ $member->nim }}</span>
                </td>
                <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="font-semibold text-slate-800 text-sm">{{ $member->nama }}</span>
                    </div>
                </td>
                <td class="px-6 py-4 hidden md:table-cell text-slate-500 text-sm">{{ $member->email }}</td>
                <td class="px-6 py-4 hidden lg:table-cell text-slate-500 text-sm max-w-xs truncate">{{ $member->alamat }}</td>
                <td class="px-6 py-4 text-center">
                    <span class="badge {{ $member->activeLoans()->count() > 0 ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500' }}">
                        {{ $member->activeLoans()->count() }} aktif
                    </span>
                </td>
                <td class="px-6 py-4">
                    <div class="flex items-center justify-center gap-2">
                        <a href="{{ route('members.show', $member) }}" class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                        </a>
                        <a href="{{ route('members.edit', $member) }}" class="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors">
